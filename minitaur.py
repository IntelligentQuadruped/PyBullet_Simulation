...
class Minitaur:
  ...
  def reset(self):
    self.quadruped = p.loadURDF("%s/quadruped/minitaur.urdf" % self.urdfRootPath, basePosition=[0,0,0.2], globalScaling=1)
    self.kp = 1
    self.kd = 0.1
    self.maxForce = 3.5
    self.nMotors = 8
  ...
  def getBasePosition(self): #gets centre of mass, body is [.33 0.10 .07] dimensions
  ...
  def applyAction(self, motorCommands, direction):
    motorCommandsWithDir = np.multiply(motorCommands, direction)
